...
local function toggleBringPlayer(playerName, state)
    ...
    if not target then
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Bring", Text = "ไม่พบผู้เล่น: "..playerName, Duration = 3
        })
        BringActive = false
        return
    end
...
local function getBringPlayerNames()
    local names = {}
...
bringDropdown = PlayerTab:Dropdown({
    Title = "เลือกผู้เล่น",
    Values = getBringPlayerNames(),
    Value = getBringPlayerNames()[1] or "ไม่มีผู้เล่นอื่น",
    Multi = false,
...
PlayerTab:Button({
    ...
    Callback = function()
        local names = getBringPlayerNames()
        bringDropdown:Refresh(names, true)
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Bring", Text = "อัปเดตรายชื่อเรียบร้อย ("..#names.." คน)", Duration = 1
        })
    end
})
    ...
    Callback = function(state)
        if state then
            if not selectedBringPlayer or selectedBringPlayer == "" or selectedBringPlayer == "ไม่มีผู้เล่นอื่น" then
                game:GetService("StarterGui"):SetCore("SendNotification", {
                    Title = "Bring", Text = "กรุณาเลือกผู้เล่นก่อน", Duration = 2
                })
            ...
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Bring", Text = "กำลังดึง "..selectedBringPlayer, Duration = 2
            })
        else
            bringActive = false
            toggleBringPlayer(selectedBringPlayer, false)
            game:GetService("StarterGui"):SetCore("SendNotification", {
                Title = "Bring", Text = "หยุดดึง "..selectedBringPlayer, Duration = 2
            })
        end
    end
})
...
Players.PlayerAdded:Connect(function()
    task.wait(0.5)
    pcall(function() bringDropdown:Refresh(getBringPlayerNames(), true) end)
end)
...
Players.PlayerRemoving:Connect(function(player)
    ...
    if bringActive and selectedBringPlayer == player.Name then
        ...
        toggleBringPlayer(selectedBringPlayer, false)
        game:GetService("StarterGui"):SetCore("SendNotification", {
            Title = "Bring", Text = "ผู้เล่นออกจากเกม หยุดดึง", Duration = 2
        })
    end
end)
